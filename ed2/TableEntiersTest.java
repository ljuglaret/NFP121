...
public class TableEntiersTest extends junit.framework.TestCase{
  ...
  public void testRetrait(){
    try{
      ...
      table.ajouter(new Entier(1));// [0,1]
      assertEquals(2, table.taille());
      table.retirer(new Entier(0));// [1]
      assertEquals(1, table.taille());
      ...
      table.ajouter(new Entier(0));
      table.ajouter(new Entier(2));//[0,1,0,2]
  ...
  public void testIterator() {
    try{
      ...
      for(int i = 0 ; i < 6; i++){
        System.out.println(new Entier(i+10).getEntier() + " " + it.next().getEntier());
